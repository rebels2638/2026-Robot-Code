 ...
 * Simulates 2D projectile motion with gravity and logs the position to
 * AdvantageScope.
...
public class ProjectileVisualizer extends Command {
     ...
     * @param launchPose     Initial pose of the shooter (includes position and
     *                       orientation)
     */
    ...
    public boolean isFinished() {
        // End when projectile starts going down (vertical velocity becomes negative)
        // vz(t) = vz0 - g * t, so vz <= 0 when t >= vz0 / g
        double t = Timer.getFPGATimestamp() - startTime;
        double currentVz = vz0 - GRAVITY * t;
        return currentVz <= 0 && currentPosition.getMeasureZ().baseUnitMagnitude() < finalZ;
